...
import { X, ArrowRight, ShieldCheck, Minus, Plus } from "lucide-react";

export default function CartPage() {
  const [mounted, setMounted] = useState(false);
  
  // Fake cart initial state populated from PRODUCTS
  const [cartItems, setCartItems] = useState([
    { ...PRODUCTS[0], quantity: 1, size: "42" },
    { ...PRODUCTS[1], quantity: 2, size: "44" }
  ]);
           ...
           <MotionViewport className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12">
              ...
              <div className="flex flex-col gap-2 md:text-right">
                 ...
                 <p className="text-[12px] font-bold uppercase tracking-widest italic text-zinc-500">
                    {cartItems.length} {cartItems.length === 1 ? 'Artifact' : 'Artifacts'} Pending
                 </p>
              </div>
           </MotionViewport>
        </section>
                       ...
                       >
                          ...
                          <div className="flex flex-col justify-between flex-1 py-1">
                             <div className="flex justify-between items-start gap-4">
                                ...
                                <button type="button" onClick={() => removeItem(item.sku)} className="text-zinc-300 hover:text-red-500 transition-colors" aria-label="Remove item">
                                   <X size={20} />
                                </button>
                             </div>
                             ...
                             <div className="flex justify-between items-end mt-6 sm:mt-0">
                                ...
                                <div className="flex items-center border border-black/20 bg-white">
                                   <button onClick={() => updateQuantity(item.sku, -1)} className="w-8 h-8 flex items-center justify-center text-black hover:bg-black hover:text-white transition-colors">
                 ...
                 {cartItems.length > 0 ? (
                    <>
                       <div className="space-y-4 text-[12px] font-bold uppercase tracking-widest text-zinc-500 italic">
                          ...
                          {shipping > 0 && (
                             <p className="text-[9px] text-zinc-400 mt-1 normal-case not-italic tracking-normal">
                                Add ${300 - subtotal} more for complimentary worldwide shipping.
                             </p>
                          )}
                       </div>
                       ...
                       <button className="w-full bg-black text-white py-5 text-[14px] font-black hover:bg-zinc-800 transition-all uppercase tracking-[0.2em] italic group flex items-center justify-center gap-4 relative overflow-hidden mt-4">
                       ...
                       <div className="text-center pt-2">
                          <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 italic">
                             Taxes calculated at secure checkout. All sales final.
                          </p>
                       </div>
                    </>
                 ) : (
